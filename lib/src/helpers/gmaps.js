 ...
 */
export const getPlacesService = (key) => new Promise((resolve, reject) => {
  try {
    const { googlePlaceService } = window;
...
export const findPlacesFromQuery = (key, query) => new Promise((resolve, reject) => {
  ...
  const request = { query, fields: ['ALL'] };
  const handleServicePromise = (service) => { service.findPlaceFromQuery(request, callback); };

  return getPlacesService(key)
    .then(handleServicePromise)
    .catch(reject);
});

export const getPlaceDetails = (key, placeId) => new Promise((resolve, reject) => {
  ...
  const request = { placeId, fields: ['ALL'] };
  const handleServicePromise = (service) => { service.getDetails(request, callback); };
